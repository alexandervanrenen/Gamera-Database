...
TEST(ExternalSort, Simple_plus_one) {
   EXPECT_TRUE(dbiu::createTestFile("bin/externalsortinput", 1<<10 + 1, [&](uint64_t i) {return rand();}));
   dbi::ExternalSort sorty;
   sorty.simpleSort("bin/externalsortinput", "bin/externalsortoutput");
...
TEST(ExternalSort, Simple_minus_one) {
   EXPECT_TRUE(dbiu::createTestFile("bin/externalsortinput", 1<<10 - 1, [&](uint64_t i) {return rand();}));
   dbi::ExternalSort sorty;
   sorty.simpleSort("bin/externalsortinput", "bin/externalsortoutput");
...
TEST(ExternalSort, ComplexBig_1024_Pages) {
   EXPECT_TRUE(dbiu::createTestFile("bin/data", 1<<size, [&](uint64_t i) {return rand();}));
   dbi::ExternalSort sorty;
   sorty.complexSort(std::string("bin/data"), 1024*1024, 1024);
